...
/**
 * Add high-margin technology products (cPanel — no Terminal / no artisan)
 *
 * Exact VAT-inclusive prices. No specialist 15% top-up. Safe to re-run.
 *
 * 1. Git pull latest main (or copy the new files into urbanfocus/)
 * 2. Copy this file to public_html/sync-high-margin.php
 * 3. Edit SYNC_KEY below to a long secret (16+ characters)
 * 4. Preview: https://www.urbanfocus.co.za/sync-high-margin.php?key=YOUR_SECRET&preview=1
 * 5. Run:    https://www.urbanfocus.co.za/sync-high-margin.php?key=YOUR_SECRET
 * 6. DELETE public_html/sync-high-margin.php when done
 */

declare(strict_types=1);

const SYNC_KEY = 'CHANGE-ME-sync-high-margin-secret';
...
echo "Urban Focus high-margin catalog sync\n";
...
if (! class_exists(App\Services\HighMarginCatalogService::class)) {
    exit("HighMarginCatalogService is missing. Pull latest main first.\n");
}

$catalog = $app->make(App\Services\HighMarginCatalogService::class);
...
echo "\nDELETE public_html/sync-high-margin.php now.\n";
