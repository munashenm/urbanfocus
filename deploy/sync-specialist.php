...
/**
 * Add specialist technology products (cPanel — no Terminal / no artisan)
 *
 * Skips any SKU or model already on the store. Safe to re-run.
 * Applies the specialist price top-up and South African SEO descriptions on products we added.
 *
 * 1. Git pull latest main (or copy the new files into urbanfocus/)
 * 2. Copy this file to public_html/sync-specialist.php
 * 3. Edit SYNC_KEY below to a long secret (16+ characters)
 * 4. Preview: https://www.urbanfocus.co.za/sync-specialist.php?key=YOUR_SECRET&preview=1
 * 5. Run:    https://www.urbanfocus.co.za/sync-specialist.php?key=YOUR_SECRET
 * 6. DELETE public_html/sync-specialist.php when done
 */

declare(strict_types=1);

const SYNC_KEY = 'CHANGE-ME-sync-specialist-secret';
...
echo "Urban Focus specialist catalog sync\n";
...
if (! class_exists(App\Services\SpecialistCatalogService::class)) {
    exit("SpecialistCatalogService is missing. Pull latest main first.\n");
}

$catalog = $app->make(App\Services\SpecialistCatalogService::class);
...
echo "\nDELETE public_html/sync-specialist.php now.\n";
